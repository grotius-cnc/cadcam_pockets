...
//    glEnd();
    ...
    //self intersect points
    glLineWidth(1);
    glPointSize(5);
    glColor4d(255,0,0,255);
    glBegin(GL_POINTS);
    for(unsigned int i=0; i<cam.size(); i++){  //spline points
            for(unsigned int j=0; j<cam[i].intersect.size(); j++){
                glVertex2d(cam[i].intersect.at(j).x,cam[i].intersect.at(j).y);
        }
    }
    glEnd();
